class CanchasController < ApplicationController
  ...
  def new
  @tiempo_array = Cancha.find(:all).collect {|p| [ p.tiempo, p.id ] }
     @diasem_array = Cancha.find(:all).collect {|p| [ p.diasem, p.id ] }
	@local_array = Local.find(:all).collect {|p| [ p.name, p.id ] }
    @cancha = Cancha.new
